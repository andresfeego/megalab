...
public class OpAbonoXEmpresa extends JDialog {
	...
	public OpAbonoXEmpresa(Principal principal, Usuario usuario) {
		...
		lblinicio = new clrLabel("Abonos por empresa", 2, true);
		...
		scrollPane.setBounds(10, 157, 625, 146);
		...
		panelPaciente.setBounds(10, 77, 645, 375);
		panelPaciente.setLayout(null);
		contentPane.add(panelPaciente);
		panelPaciente.add(scrollPane);

		txtRazonSocial = new ClrCuadroDeTexto(100,false,"Raz\u00F3n social",true);
		txtRazonSocial.setFocusCycleRoot(false);
		txtRazonSocial.setBounds(347, 22, 288, 25);
		txtRazonSocial.addFocusListener(new FocusListener() {
			...
			public void focusGained(FocusEvent e) {
				if (lb3==null) {
					lb3= new lbEmpresaXrazon(txtRazonSocial, esta, panelPaciente, esta.principal);
				}
			    ...
			    new PropertyChangeListener() {
			        ...
			        public void propertyChange(PropertyChangeEvent e) {
			        	...
			        	if (empresaSeleccion!=null) {
							llenarEmpresa(empresaSeleccion);
						}
			        }
			    });
		...
		txtCodEmpresa = new ClrCuadroDeTexto(11,true,"C\u00F3digo empresa",true);
		txtCodEmpresa.setFocusCycleRoot(false);
		txtCodEmpresa.setBounds(10, 22, 162, 25);
		txtCodEmpresa.addFocusListener(new FocusListener() {
			...
			public void focusGained(FocusEvent e) {
				if (lb1==null) {
					lb1=new lbEmpresaXcod(txtCodEmpresa, esta, panelPaciente, esta.principal);
				}
				
			}
		});
		panelPaciente.add(txtCodEmpresa);

		btnAbonarXFactura = new btnRedondo("Abonar por factura", new Rectangle(48, 172,121,50), 4);
		btnAbonarXFactura.setSelected(true);
		btnAbonarXFactura.setBounds(441, 314, 194, 50);
		...
		lblNumeroFacturas.setBounds(347, 118, 119, 28);
		...
		clrLabel__8.setBounds(334, 118, 19, 28);
		...
		lblSaldo.setBounds(234, 118, 103, 28);
		panelPaciente.add(lblSaldo);
		
		clrLabel clrLabel__10 = new clrLabel("Esta empresa presenta un saldo total de ", 1);
		...
		clrLabel__10.setBounds(10, 118, 230, 28);
		panelPaciente.add(clrLabel__10);
		
		txtNitEmpresa = new ClrCuadroDeTexto(13,false,"Nit empresa",true);
		txtNitEmpresa.setFocusCycleRoot(false);
		txtNitEmpresa.setBounds(178, 22, 162, 25);
		txtNitEmpresa.addFocusListener(new FocusListener() {
			...
			public void focusGained(FocusEvent e) {
				if (lb2==null) {
					lb2=new lbEmpresaXnit(txtNitEmpresa, esta, panelPaciente, esta.principal);
				}
				
			}
		});
		panelPaciente.add(txtNitEmpresa);

		clrPanelBordes clrPanelBordes_ = new clrPanelBordes(false);
		clrPanelBordes_.setBounds(10, 463, 645, 96);
	...
	public void llenarEmpresa(Empresa empresa) {
		...
		try {
			...
			lblincorrectos.setText("La fecha final de busqueda es mayor  a la inicial");
				...
				DatosAbono=conexionBusqueda.getInstance().facturasDabonoXEmpresa(empresa.getIdEmpresa()+"",fchHasta.getDate(),fchDesde.getDate());
	...
	public void llenarTabla(ArrayList<DatosAbono> datos){
		...
		DatosAbono DA=null;
			...
			lblNumeroFacturas.setText((datos.size())+" facturas ");
				if (datos.size()>0) {
					//  esto es paraponer el label de fecha de facturas        lblFechaFacturas.setText(DA.getFechaYHora()+"");
			}
	...
	public void Abonar(){
		if (txtAbono.getText().equals(txtAbono.getLabel())||cbFormaPago.getSelectedIndex()==0||TM.getRowCount()<0) {
			...
			AbonoXEmpresa abono=new AbonoXEmpresa(empresaSeleccion.getIdEmpresa(), cbFormaPago.getSelectedIndex(), Integer.parseInt(txtAbono.getText()),auxiObs,DatosAbono,auxiNumeroTarjeta);
			if (conexion.getInstance().nuevoAbonoXEmpresa(esta, abono)) {
				principal.registrarAccion("Abono a facturas de empresa ' "+empresaSeleccion.getRazonSocial()+" ' por $ "+txtAbono.getText());
				dispose();
			}else{
