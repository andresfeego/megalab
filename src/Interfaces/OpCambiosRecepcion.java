...
public class OpCambiosRecepcion extends JDialog {
	...
	private  OpCambiosRecepcion esta;
	private clrLabel lblTarifaActual;

	public OpCambiosRecepcion(Principal principal, Usuario usuario) {
		...
		Siguiente = new btnRedondo("Cambiar", new Rectangle(48, 172,121,50),3);
		Siguiente.setSelected(true);
		Siguiente.setBounds(352, 592, 169, 50);
	 ...
	 if (idRecepcion==-1) {
		...
		if (fchOrden.getDate()==null) {
			...
			if (fchMuestra.getDate()==null) {
				lblincorrectos.setText("Sin fecha de toma de la muestra");
			} else {
				...
				int auxiCodpyp=-1;if(cbPlanesPYP.getSelectedIndex()!=0){auxiCodpyp=Integer.parseInt(ST2[0]);};
				
				String[] sala;
				int  auxiCodSala=-1;
				if(cbSalas.getSelectedIndex()!=0){
					String[] ST3=cbSalas.getSelectedItem().toString().split(" • ");
				sala=conexionBusqueda.getInstance().SalaXsigla(ST3[0]);
				auxiCodSala=Integer.parseInt(sala[0]);
				}
				...
				int auxiCodAmbito=-1;if(cbAmbito.getSelectedIndex()!=0){auxiCodAmbito=Integer.parseInt(ST4[0]);};
				
				String[] ST5= cbFinalidad.getSelectedItem().toString().split(" • ");
				int auxiCodFinalidad=-1;if(cbFinalidad.getSelectedIndex()!=0){auxiCodFinalidad=Integer.parseInt(ST5[0]);};
			...
			Recepcion recepcion = new Recepcion(idRecepcion,paciente, empresa, tarifa, auxisede, auxiNumeroMuestra, auxiAutorizacion, auxiNumeroOrden, auxiFechaOrden, -1, auxiCodpyp, auxiCodSala, auxiCama, auxiTipoUsuario, auxiCodAmbito, auxiCodFinalidad, auxiCodMedico, auxiFechaMuestra, null, auxiobservaciones,0,"");
			
			if (conexion.getInstance().editarRecepcion(esta,recepcion)) {
				esta.principal.registrarAccion("Edición de datos basicos en recepción #: '" + recepcion.getIdREcepcion()+"'");
				dispose();
			}else{
