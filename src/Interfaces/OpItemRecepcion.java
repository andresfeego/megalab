...
public class OpItemRecepcion extends JDialog {
	...
	public OpItemRecepcion(Principal principal, Usuario usuario, Recepcion recepcion, OpRecepcion opRecepci) {
		...
		lblinicio = new clrLabel("Agregar examenes a recepción", 2, true);
...
table.setDefaultRenderer(Object.class, new clrRenderTablaItemRecepcion());
		...
		clrLabel_1 = new clrLabel("Recepción # "+String.format("%05d", (conexionBusqueda.getInstance().ultimaRecepcion())+1), 1);
	...
	public void llenarPerfilExamen(PerfilExamenes Pexamen) {
		...
		if (perfilPorAgregar) {
			...
			ventanaPregunta pregunta = new ventanaPregunta(OpItemRecepcion.this.principal, "No has agregado los examenes del perfil a esta recepción, deseas agregarlos ahora", "Si", "No");
		...
		for (int i = 0; i < listaExamenes.size(); i++) {
			...
			if (validarExamenEnFactura(Ex.getSigla())) {
				lblincorrectos.setText("Examen ya existente en la recepción");
			} else {
	...
	public void guargarRecepcion(){
		calcularTotalExamenes();
		if (TM.getRowCount()<1) {
			lblincorrectos.setText("No has agregado examenes para esta recepción");
		} else {
			if (cbTipoPago.getSelectedIndex()==0) {
				lblincorrectos.setText("No has agregado tipo de pago para esta recepción");
			} else {
				if (cbFormaPago.getSelectedIndex()==0) {
					lblincorrectos.setText("No has agregado forma de pago para esta recepción");
				} else {
			...
			if (txtAbono.getText().equals("") ||txtAbono.getText().equals("Abono") ) {
				...
				ventanaPregunta pregunta = new ventanaPregunta(OpItemRecepcion.this.principal, "Estas generando una recepción sin abono a la factura, estas seguro de que deseas continuar", "Si", "No");
				if (pregunta.escuchar()) {
					...
					if (conexion.getInstance().nuevoRecepcion(esta, RC)) {
						esta.principal.registrarAccion("Creación de recepción '"+(conexionBusqueda.getInstance().ultimaRecepcion())+"'");
