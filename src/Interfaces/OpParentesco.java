...
public class OpParentesco extends JDialog {
	...
	private OpParentesco esta;
	private lbParentescoXNombre lb1;
	private lbParentescoXid lb2;
	 ...
	 * Create the frame.
	 */
	public OpParentesco(Principal pri, Usuario usuario, String Accion) {
		super(pri, true);
		System.out.println();
	...
	setBounds(100, 100, 389, 271);
		...
		if (Accion.endsWith("Agregar")) {
			...
			lblinicio = new clrLabel("Agregar parentesco", 2, true);
			...
			txtNombre = new ClrCuadroDeTexto(100,false,"Parentesco",1);
			...
			if (Accion.equals("Modificar")) {
				...
				contentPane = new clrPanelBordes(true);
				contentPane.setLayout(null);
				lblinicio = new clrLabel("Modificar parentesco", 2, true);
				lblinicio.setHorizontalAlignment(SwingConstants.CENTER);
				lblinicio.setAlignmentX(Component.CENTER_ALIGNMENT);
				lblinicio.setBounds(10, 11, 369, 28);
				...
				txtNombre = new ClrCuadroDeTexto(100,false,"Parentesco",true,1);
				txtNombre.setBounds(40, 109, 306, 28);
				txtNombre.addFocusListener(new FocusListener() {
					...
					public void focusGained(FocusEvent e) {
						if (lb1==null) {
							lb1=new lbParentescoXNombre(txtNombre, esta, esta.principal);
						}
				...
				contentPane = new clrPanelBordes(true);
				contentPane.setLayout(null);
				lblinicio = new clrLabel("Eliminar parentesco", 2, true);
				lblinicio.setHorizontalAlignment(SwingConstants.CENTER);
				lblinicio.setAlignmentX(Component.CENTER_ALIGNMENT);
				lblinicio.setBounds(10, 11, 369, 28);
				...
				txtNombre = new ClrCuadroDeTexto(100,false,"Parentesco",true);
				txtNombre.setBounds(40, 109, 306, 28);
				txtNombre.addFocusListener(new FocusListener() {
	...
	public void  agregarParentesco(){
		if (txtNombre.getText().equals("")||txtNombre.getText().equals(txtNombre.getLabel())) {
			lblincorrectos.setText("No has introducido ningun parentesco");
		...
		if (conexion.getInstance().nuevoParentesco(esta,txtNombre.getText())) {
			principal.registrarAccion("Creación de parentesco: " + txtNombre.getText());
			dispose();
		}else{
	...
	public void modificarCiudad(){
		if (txtNombre.getText().equals(parentescoAntigua)) {
			...
			if (idParentesco==-1) {
				lblincorrectos.setText("No has escogido ningun parentesco");
			...
			if (conexion.getInstance().editarParentesco(esta,idParentesco,txtNombre.getText())) {
				principal.registrarAccion("Modificación de  parentesco: " + txtNombre.getText());
				dispose();
	...
	public void eliminarCiudad(){
		...
		if (conexion.getInstance().eliminarParentesco(esta,idParentesco)) {
			principal.registrarAccion("Eliminación de parentesco: " + parentescoAntigua);
			dispose();
		}else{
			lblincorrectos.setText("Error  en la conexion con la base de datos");
		}
