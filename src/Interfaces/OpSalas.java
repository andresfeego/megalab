...
public class OpSalas extends JDialog {
	...
	private OpSalas esta;
	private lbSalaXSigla lb1;
	private lbSalaXnombre lb2;
	 ...
	 * Create the frame.
	 */
	public OpSalas(Principal pri, Usuario usuario, String Accion) {
		super(pri, true);
		System.out.println();
	...
	setBounds(100, 100, 389, 271);
		...
		if (Accion.endsWith("Agregar")) {
			...
			lblinicio = new clrLabel("Agregar sala", 2, true);
			...
			if (Accion.equals("Modificar")) {
				...
				contentPane = new clrPanelBordes(true);
				contentPane.setLayout(null);
				lblinicio = new clrLabel("Modificar sala", 2, true);
				lblinicio.setHorizontalAlignment(SwingConstants.CENTER);
				lblinicio.setAlignmentX(Component.CENTER_ALIGNMENT);
				lblinicio.setBounds(10, 11, 369, 28);
				...
				txtNombre.addFocusListener(new FocusListener() {
					...
					public void focusGained(FocusEvent e) {
						if (lb2==null) {
							lb2=new lbSalaXnombre(txtNombre, esta, esta.principal);
						}
				...
				txtSigla.addFocusListener(new FocusListener() {
					...
					public void focusGained(FocusEvent e) {
						if (lb1==null) {
							lb1=new lbSalaXSigla(txtSigla, esta, esta.principal);
						}
				...
				contentPane = new clrPanelBordes(true);
				contentPane.setLayout(null);
				lblinicio = new clrLabel("Eliminar sala", 2, true);
				lblinicio.setHorizontalAlignment(SwingConstants.CENTER);
				lblinicio.setAlignmentX(Component.CENTER_ALIGNMENT);
				lblinicio.setBounds(10, 11, 369, 28);
	...
	public void llenar(int  idSalaa,String nombre,String sigla) {
	...
	public void  agregarSala(){
		if (txtNombre.getText().equals("")||txtNombre.getText().equals(txtNombre.getLabel())||txtSigla.getText().equals(txtSigla.getLabel())) {
			if(txtNombre.getText().equals(txtNombre.getLabel())){lblincorrectos.setText("No has introducido un nombre para la sala");};
			if(txtSigla.getText().equals(txtSigla.getLabel())){lblincorrectos.setText("No has introducido sigla para esta sala");};
		...
		if (conexion.getInstance().nuevoSala(esta, txtSigla.getText(),txtNombre.getText())) {
			principal.registrarAccion("Creación de sala: " + txtNombre.getText());
			dispose();
		}else{
	...
	public void modificarSala(){
		if (txtNombre.getText().equals(nombreAntigua)&&txtSigla.getText().equals(siglaAntigua)) {
			...
			if (idSala==-1) {
				lblincorrectos.setText("No has seleccionado ninguna sala");
			}else{
				lblincorrectos.setText("");

				
				if (conexion.getInstance().editarSala(esta,idSala,txtNombre.getText(),txtSigla.getText())) {
					principal.registrarAccion("Modificación de  sala: " + txtNombre.getText());
					dispose();
				}else{
					lblincorrectos.setText("Error  en la conexion con la base de datos");
				}
	...
	public void eliminarCiudad(){
		if (idSala==-1) {
			lblincorrectos.setText("No has escogido ninguna sala");
		...
		if (conexion.getInstance().eliminarSala(esta,idSala)) {
			principal.registrarAccion("Eliminación de sala: " + nombreAntigua);
			dispose();
		}else{
			lblincorrectos.setText("Error  en la conexion con la base de datos");
		}
