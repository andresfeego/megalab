...
public class lbBacteriologoXnombre extends JDialog {
	 ...
	 * 
	 */
	public lbBacteriologoXnombre(ClrCuadroDeTexto cuadroTexto,OpBacteriologo op,Principal pri) {
		...
		contentPane.setBorder(new LineBorder(Colores.clrPrincipal, 2));		scrollPane.transferFocus();
	...
	public void listaok(){
		...
		try {	
			Bacteriologo bacte=conexionBusqueda.getInstance().bacteriologoXid(tokens[0]);
			esta.setBcateriologo(bacte);
			op.llenar(bacte);
		...
		if (busqueda.equals("")) {
			list.setModel(new AbstractListModel() {
				...
				list.setModel(new AbstractListModel() {
					...
					String[] values=conb.busquedaBacteriologoXnombre(cuadrodetexto.getText());
		...
		public Bacteriologo getBActeriologo() {
			return bacteriologo;
		}

		public void setBcateriologo(Bacteriologo bacte) {
			this.bacteriologo = bacte;
		}
