...
public class lbExamenXnombre extends JDialog {

	private clrPanelBordes contentPane;
	private Principal principal;
	private Examen examen;
	private lbExamenXnombre esta = null;
	private CerrarBusquedaInactiva crono = null;
	private ClrCuadroDeTexto cuadrodetexto;
	private JList list;
	private OpExamen op;
	private OpItemRecepcion op1;
	private OpPerfiles op2;
	private OpItemCotizacion op3;
	private OpResXSeccion op4;
			private  int s=-1;
	 ...
	 * 
	 */
	public lbExamenXnombre(ClrCuadroDeTexto cuadroTexto,OpItemRecepcion op,clrPanelBordes panel,Principal pri) {
	...
	public lbExamenXnombre(ClrCuadroDeTexto cuadroTexto, OpExamen op, Principal pri) {
	...
	public lbExamenXnombre(ClrCuadroDeTexto cuadroTexto,OpPerfiles op,clrPanelBordes panel,Principal pri) {
	...
	public lbExamenXnombre(ClrCuadroDeTexto cuadroTexto,OpItemCotizacion op,clrPanelBordes panel,Principal pri) {
	...
	public lbExamenXnombre(ClrCuadroDeTexto cuadroTexto,OpResXSeccion op,clrPanelBordes panel,Principal pri) {
	...
	public void asignarPaciente() {
		...
		examen = conexionBusqueda.getInstance().examenXcodigo(tokens[1]);
		esta.setExamen(examen);
		if (op!=null) op.llenar(examen);
		if (op1!=null)op1.llenarExamen(examen);
		if (op2!=null)op2.llenarExamen(examen);
		if (op3!=null)op3.llenarExamen(examen);
		if (op4!=null)op4.llenarExamen(examen);
	...
	public void crearBusqueda(String busqueda) {
		...
		if (busqueda.equals("")) {
			...
			list.setModel(new AbstractListModel() {
				...
				String[] values = conb.busquedaExamenXnombre(cuadrodetexto.getText());
