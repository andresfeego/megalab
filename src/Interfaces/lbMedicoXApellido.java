...
public class lbMedicoXApellido extends JDialog {
	 ...
	 * 
	 */
	public lbMedicoXApellido(ClrCuadroDeTexto cuadroTexto,OpMedico op,Principal pri) {
		...
		setBounds((int)Math.floor(op.getBounds().getX()-0-cuadroTexto.getBounds().getWidth())+100,(int)Math.floor(op.getBounds().getY()+1+cuadroTexto.getBounds().getY()),(int)Math.floor(cuadroTexto.getBounds().getWidth()), 190);
...
public lbMedicoXApellido(ClrCuadroDeTexto cuadroTexto,OpRecepcion op,clrPanelBordes panel,Principal pri) {
...
public lbMedicoXApellido(ClrCuadroDeTexto cuadroTexto,OpCambiosRecepcion op,clrPanelBordes panel,Principal pri) {
...
public lbMedicoXApellido(ClrCuadroDeTexto cuadroTexto,OpAnularRecepcion op,clrPanelBordes panel,Principal pri) {
	...
	public void listaok(){
		...
		public void asignarBacteriologo(){
		Paciente pacien=null;
		String[] tokens = list.getSelectedValue().toString().split(" • ");
		Medico medico=conexionBusqueda.getInstance().medicoXid(tokens[0]);
				esta.setMedico(medico);
				if (op!=null) op.llenar(medico);
				if (op1!=null)op1.llenarMedico(medico);
				if (op2!=null)op2.llenarMedico(medico);
				if (op3!=null)op3.llenarMedico(medico);
		...
		if (busqueda.equals("")) {
			list.setModel(new AbstractListModel() {
				...
				list.setModel(new AbstractListModel() {
					...
					String[] values=conb.busquedaMedicoXapellido(cuadrodetexto.getText());
		...
		public Medico getBActeriologo() {
			return medico;
		}

		public void setMedico(Medico medic) {
			this.medico = medic;
		}
