...
public class lbPerfilesXnombre extends JDialog {

	private clrPanelBordes contentPane;
	private Principal principal;
	private PerfilExamenes PerfilExamen;
	private  lbPerfilesXnombre esta=null;
	private  CerrarBusquedaInactiva crono=null;
	private  ClrCuadroDeTexto cuadrodetexto;
	private  JList list;
	private OpItemRecepcion op1;
	private OpPerfiles op;
	private OpItemCotizacion op2;
				private  int s=-1;
...
public lbPerfilesXnombre(ClrCuadroDeTexto cuadroTexto,OpPerfiles op,Principal pri) {
	...
	public lbPerfilesXnombre(ClrCuadroDeTexto cuadroTexto,OpItemRecepcion op,clrPanelBordes panel, clrPanelBordes panelinterno,Principal pri) {
		...
		setBounds((int)Math.floor(op1.getBounds().getX()+cuadroTexto.getX()+panel.getBounds().getX()+panelinterno.getBounds().getX()),(int)Math.floor(op1.getBounds().getY()+cuadroTexto.getHeight()+4+cuadroTexto.getBounds().getY()+panel.getBounds().getY()+panelinterno.getBounds().getY()),273, 190);
		setAlwaysOnTop(true);
		this.setUndecorated(true);
		contentPane = new clrPanelBordes(false);
		contentPane.setLayout(null);
...
public lbPerfilesXnombre(ClrCuadroDeTexto cuadroTexto,OpItemCotizacion op,clrPanelBordes panel, clrPanelBordes panelinterno,Principal pri) {
		...
		setBounds((int)Math.floor(op2.getBounds().getX()+cuadroTexto.getX()+panel.getBounds().getX()+panelinterno.getBounds().getX()),(int)Math.floor(op2.getBounds().getY()+cuadroTexto.getHeight()+4+cuadroTexto.getBounds().getY()+panel.getBounds().getY()+panelinterno.getBounds().getY()),273, 190);
		setAlwaysOnTop(true);
		this.setUndecorated(true);
		contentPane = new clrPanelBordes(false);
		contentPane.setLayout(null);
	...
	public void listaok(){
		...
		PerfilExamenes PerfilExamen=null;
		String[] tokens = list.getSelectedValue().toString().split(" • ");
		PerfilExamen=conexionBusqueda.getInstance().PerfilXcodigo(tokens[0]);
				esta.setExamen(PerfilExamen);
				if(op!=null)op.llenarPerfil(PerfilExamen);
				if(op1!=null)op1.llenarPerfilExamen(PerfilExamen);
				if(op2!=null)op2.llenarPerfilExamen(PerfilExamen);
		...
		if (busqueda.equals("")) {
			list.setModel(new AbstractListModel() {
				...
				list.setModel(new AbstractListModel() {
					...
					String[] values=conb.busquedaPerfilesXnombre(cuadrodetexto.getText());
		...
		public PerfilExamenes getPaciente() {
			return PerfilExamen;
		}

		public void setExamen(PerfilExamenes Pexamen) {
			this.PerfilExamen = Pexamen;
		}
