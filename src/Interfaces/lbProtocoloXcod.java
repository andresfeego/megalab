...
public class lbProtocoloXcod extends JDialog {
	 ...
	 * 
	 */
	public lbProtocoloXcod(ClrCuadroDeTexto cuadroTexto,OpProtocolo  op,Principal pri) {
	...
	public lbProtocoloXcod(ClrCuadroDeTexto cuadroTexto,OpExamen  op,clrPanelBordes panel,Principal pri) {
		...
		setRootPaneCheckingEnabled(false);
		setLocationRelativeTo(op);
		setBounds((int)Math.floor(op.getBounds().getX()+cuadroTexto.getX()+panel.getBounds().getX()),(int)Math.floor(op.getBounds().getY()+cuadroTexto.getHeight()+4+cuadroTexto.getBounds().getY()+panel.getBounds().getY()),273, 190);
		setAlwaysOnTop(true);
		this.setUndecorated(true);
		contentPane = new clrPanelBordes(false);
		contentPane.setLayout(null);
	...
	public void listaok(){
		...
		protocolo protocolo=conexionBusqueda.getInstance().protocoloXnombre(tokens[1]);
		...
		if (busqueda.equals("")) {
			list.setModel(new AbstractListModel() {
				...
				list.setModel(new AbstractListModel() {
					...
					String[] values=conb.busquedaProtocoloXcod(cuadrodetexto.getText());
