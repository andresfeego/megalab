 ...
 *
 */
public class CuentaCobroXEmpresa {
	...
	private ArrayList<itemCuenta> itemsCuenta;
	...
	private int contadorFcaturas=0;
	private int contadorValor=0;
	private int contadorAbono=0;
	private int contadorSaldo=0;
	
	
	
	public CuentaCobroXEmpresa(Empresa emprea,int subgrupo ,ArrayList<DatosAbono> items,int saldo,boolean imprimir,boolean detallado,Bacteriologo bacteriologo) {
		...
		Document document = new Document(Colores.getRectangleItext(Colores.tamHojaListados), 35, 30, 40, 50);
		try {
		...
		// Cargar codigo de barras
			...
			document.add(numeroRecepcion);
				...
				cliente.add(new Phrase(empresa.getRazonSocial().toUpperCase(),Colores.fuenteTitulosBold));
				cliente.add(new Phrase(Chunk.NEWLINE));
				cliente.add(new Phrase("Nit : "+empresa.getDocEmpresa(),Colores.fuenteContenidos));
				cliente.add(new Phrase(Chunk.NEWLINE));
				...
				if (this.detallado) {
					document.newPage();
							...
							itemsCuenta=new ArrayList<itemCuenta>();
					Paragraph Anexo = new Paragraph();
					Anexo.setAlignment(Element.ALIGN_JUSTIFIED);
					Anexo.add(new Phrase("ANEXO 1 - Detalles de cuenta de cobro número "+(conexionBusqueda.getInstance().ultimaCuentaCobro()+1)+" para la empresa "+empresa.getRazonSocial(),Colores.fuenteTitulosBold));
					if (subgrupo!=-1) {
						Anexo.add(new Phrase("  y para subgrupo de empresa "+ conexionBusqueda.getInstance().grupoEmpresaXID(""+subgrupo).getNombreGrupo(),Colores.fuenteTitulosBold));
					...
					ArrayList<Integer> listaGrupos=separarXsubgrupos();
					...
					for (int i = 0; i < listaGrupos.size(); i++) {
							...
							GruposEmpresas GE;
							GE=new GruposEmpresas(-1, "Sin sub grupo", "1", "0");
							if (listaGrupos.get(i)!=-1) {
								GE=conexionBusqueda.getInstance().grupoEmpresaXID(""+listaGrupos.get(i));

							}
						
			if (GE.getIdGrupo()==subgrupo || subgrupo ==-1) {
				
				PdfPCell cellTituloDetalles = new PdfPCell(new Phrase("Sub grupo de empresa: "+GE.getNombreGrupo(),Colores.fuenteContenidosBold));
				...
				for (int j = 0; j < items.size(); j++) {
					...
					if (listaGrupos.get(i)==RCI.getRecepcion().getCodSubEmpresa()) {
...
//////////////////////////////////////////////////////////////////////
				...
				PdfPCell cellTitTotalGrupo = new PdfPCell(new Phrase("Totales para sub grupo de empresa: "+GE.getNombreGrupo(),Colores.fuenteContenidosBold));
				...
				document.add(sep);
					...
					float[] medidaCeldas2 = {1.2f, 3f, 1.2f,1.3f,1.3f};
					tableDetalles3.setWidths(medidaCeldas2);
			...
			cuentaCobro cuenta=new cuentaCobro(-1, (new ObtenerFechaColombia()).fechaColombia(), items.get(items.size()-1).getFechaYHora(), items.get(0).getFechaYHora(), empresa.getCodEps(), empresa.getIdEmpresa()+"", empresa.getRazonSocial(), subgrupo, "0", contadorGranTotalAbonos+"", 0, contadorGranTotalSaldos, contadorSeervicios, itemsCuenta, pdf);
	...
	public ArrayList<Integer> separarXsubgrupos(){
		ArrayList<Integer> salida=new ArrayList<Integer>();
		
		salida.add(conexionBusqueda.getInstance().RCXid(items.get(0).getCodRecepcion()).getRecepcion().getCodSubEmpresa());
		for (int i = 1; i < items.size(); i++) {
			DatosAbono DA=items.get(i);
			RecepcionCompleta RC=conexionBusqueda.getInstance().RCXid(DA.getCodRecepcion());
			int conta=0;
			for (int j = 0; j < salida.size(); j++) {
				System.err.println(RC.getRecepcion().getCodSubEmpresa()+" - "+salida.get(j));
				
				if (RC.getRecepcion().getCodSubEmpresa()==salida.get(j)) {
					conta++;
			...
			if (conta==0) {
					salida.add(conexionBusqueda.getInstance().RCXid(DA.getCodRecepcion()).getRecepcion().getCodSubEmpresa());
				}
		}
